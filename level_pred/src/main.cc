...
void finish_warmup()
{
    ...
    for (uint32_t i=0; i<NUM_CPUS; i++) {
        ...
        ooo_cpu[i].MD.LATENCY   = MD_LATENCY;
    }
...
int main(int argc, char** argv)
{
	...
	ooo_cpu[i].L1I.l1i_prefetcher_cache_fill = cpu_l1i_prefetcher_cache_fill;
        ...
        ooo_cpu[i].MD.cpu = i;
        ooo_cpu[i].MD.cache_type = IS_MD;
        ooo_cpu[i].MD.fill_level = FILL_MD;
        ooo_cpu[i].MD.lower_level = &uncore.MDRAM;
        ...
        // OFF-CHIP MDRAM
        uncore.MDRAM.fill_level = FILL_MDRAM;
        uncore.MDRAM.upper_level_icache[i] = &ooo_cpu[i].MD;
        uncore.MDRAM.upper_level_dcache[i] = &ooo_cpu[i].MD;
        for (uint32_t i=0; i<MDRAM_CHANNELS; i++) {
            uncore.MDRAM.RQ[i].is_RQ = 1;
            uncore.MDRAM.WQ[i].is_WQ = 1;
        }
