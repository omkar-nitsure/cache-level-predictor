...
unsigned char METADATA::popular_level_detector(){
    uint32_t max_access = 0;
    uint32_t max_access_level = 0;
    for(uint32_t i=0; i<NUM_CPUS; i++){
        if(sim_access[i][0] > max_access){
        max_access = sim_access[i][0];
        max_access_level = i;
        }
    }
    unsigned char popular_level = 0;
    popular_level = max_access_level & 0x3;
...
void CACHE::handle_read(METADATA *MD, METADATA_MEM *MD_MEM)
{
    // handle read
                ...
                { // this is a new miss
                    ...
                    else
                    {
                        if (cache_type == IS_L1D)
                        {
                            ...
                            uint32_t metadata_set = MD->get_set_md(RQ.entry[index].address >> LOG2_MD_SIZE);
                            int metadata_way = MD->check_hit_md(&RQ.entry[index]);
                            if (metadata_way >= 0)
                            {
                                // hit
                                unsigned char cache_to_fetch = block_md[metadata_set][metadata_way].data & 0x3;
                                if (cache_to_fetch == 1)
                                {
                                    // L1D miss
                                    add_mshr(&RQ.entry[index]);
                                    lower_level->add_rq(&RQ.entry[index]);
                                    // RQ.entry[index].type = RFO;
                                    // RQ.entry[index].fill_l1d = 1;
                                }
                                else if (cache_to_fetch == 2)
                                {
                                    // L2C miss
                                    add_mshr(&RQ.entry[index]);
                                ...
                                unsigned char cache_to_fetch = MD->popular_level_detector();
                                if (cache_to_fetch == 1)
                                {
                                    // L1D miss
                                    add_mshr(&RQ.entry[index]);
                                ...
                                MD->lower_level->add_rq(MD_MEM->base_add + (&RQ.entry[index]) >> LOG2_MD_SIZE);
                            }
                        }
                        else{
                            add_mshr(&RQ.entry[index]);
